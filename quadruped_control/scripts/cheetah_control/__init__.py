...
class cheetah_control():
    ...
    def go_to_zero_all(self, Kp, Kd):
        if self.check_zero_flag():
            self.go_to_zero_leg('LF_leg', self.LF_leg, Kd, Kp)
            self.go_to_zero_leg('LB_leg', self.LB_leg, Kd, Kp)
            self.go_to_zero_leg('RF_leg', self.RF_leg, Kd, Kp)
            self.go_to_zero_leg('RB_leg', self.RB_leg, Kd, Kp)
    ...
    def motor_go_to_des_pos_wrt_body(self, leg_name, motors_names, pos_des, Kd, Kp, quad_kin):
        if not self.check_zero_flag():
            q_cur = np.array([[0],
                            [0],
                            [0]], dtype = np.float)
    ...
    def go_to_desired_RPY_of_base(self, quad_kin, LF_leg_pos, RF_leg_pos, LB_leg_pos, RB_leg_pos, Kd, Kp):
        ...
        if not self.check_zero_flag():
            ...
            U_sum = {'LF':[], 'RF':[], 'LB':[], 'RB': []}
            U, flag = self.motor_go_to_des_pos_wrt_body('LF_leg', self.LF_leg, p_LF_leg_B, Kd, Kp, quad_kin)
            if flag:
                U_sum['LF'] = U
            U, flag = self.motor_go_to_des_pos_wrt_body('RF_leg', self.RF_leg, p_RF_leg_B, Kd, Kp, quad_kin)
            if flag:
                U_sum['RF'] = U
            U, flag = self.motor_go_to_des_pos_wrt_body('LB_leg', self.LB_leg, p_LB_leg_B, Kd, Kp, quad_kin)
            if flag:
                U_sum['LB'] = U
            U, flag = self.motor_go_to_des_pos_wrt_body('RB_leg', self.RB_leg, p_RB_leg_B, Kd, Kp, quad_kin)
            if flag:
                U_sum['RB'] = U
            return U_sum, 1
        return None, 0
